...
uses
  Vcl.Dialogs,
  Windows, USubtitleFile, USubtitlesRead, SysUtils, StrUtils, jclStrings;
    ...
    try
      ...
      begin
        ...
        for j := i+1 to tmpSubFile.Count-1 do
          if not CharInSet(tmpSubFile[j][1], ['0'..'9']) then //a line with a non-digit first symbols can't be DVD Subtitle System, so it's Adobe Encore DVD (Old)
          begin
      ...
      if (StringCount('{', tmpSubFile[i]) >= 2) and
         ...
         (
         (IsInteger(Copy(tmpSubFile[i], 2, Pos('}', tmpSubFile[i]) - 2)) or ((Copy(tmpSubFile[i], 2, Pos('}', tmpSubFile[i]) - 2) = '') and (i > 0) and (IsInteger(Copy(tmpSubFile[i-1], PosEx('{', tmpSubFile[i-1], 2) + 1, PosEx('}', tmpSubFile[i-1], Pos('}', tmpSubFile[i-1]) + 1) - (PosEx('{', tmpSubFile[i-1], 2) + 1)))))) and
